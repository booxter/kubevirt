...
# todo: change the job to actually configure non-empty list of PF IDs for device plugin use
kubectl --kubeconfig cluster/k8s-1.13.0-sriov/config apply -f cluster/k8s-1.13.0-sriov/manifests/sriov-config-job.yaml
sleep 10

# wait for the sriov-config-job to finish
...
docker exec -e FUNC_TEST_ARGS="--ginkgo.noColor --junit-output=/go/src/kubevirt.io/kubevirt/tests.junit.xml --ginkgo.focus=SRIOV --kubeconfig /etc/kubernetes/admin.conf" -t kube-master make functest
